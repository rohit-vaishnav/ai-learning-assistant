...
import { Sun, Moon, Sparkles, BookOpen, LogOut, LogIn, UserPlus } from 'lucide-react';

const Navbar = () => {
  const { theme, toggleTheme, selectedFile, uploadedFiles, setSelectedFile, isAuthenticated, user, handleLogout } = useApp();
  ...
  return (
    <header className="glass sticky top-0 z-40 w-full border-b border-slate-200/50 dark:border-slate-800/50 px-6 py-3 flex items-center justify-between transition-colors duration-300">
      {/* Logo */}
      <NavLink to={isAuthenticated ? "/dashboard" : "/"} className="flex items-center gap-2 font-display font-bold text-xl text-slate-800 dark:text-slate-100 hover:opacity-90">
      ...
      <div className="flex items-center gap-3">
        {/* Active file indicator (Only for authenticated users with uploads) */}
        {isAuthenticated && (
          uploadedFiles.length > 0 ? (
            ...
            <NavLink 
              to="/upload" 
              className="hidden md:flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
            >
        ...
        {isAuthenticated ? (
          <div className="flex items-center gap-3 border-l border-slate-200 dark:border-slate-800 pl-3">
            <span className="hidden sm:inline-block text-xs font-bold text-slate-700 dark:text-slate-300">
              {user?.name}
            </span>
            <button
              onClick={handleLogout}
              className="flex items-center justify-center p-2 rounded-xl text-rose-600 dark:text-rose-400 bg-rose-500/10 hover:bg-rose-500/20 transition-colors cursor-pointer"
              title="Logout"
            >
          ...
          <div className="flex items-center gap-2">
            <NavLink
              to="/login"
              className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
            ...
            <NavLink
              to="/register"
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-violet-600 hover:bg-violet-500 text-white shadow-sm transition-colors"
            >
